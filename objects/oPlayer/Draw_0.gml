...
draw_sprite_ext(sCannon,0,_x2+lengthdir_x(lerp(cannonLength-1,cannonLength,shootPercent)*image_xscale-cannonMove*1.1,drawDir),_y2+lengthdir_y(lerp(cannonLength-1,cannonLength,shootPercent)*image_yscale-cannonMove*1.1,drawDir),lerp(0.1,1,shootPercent),1,drawDir,merge_color(c_white,c_red,min(1,generalShake/2+(global.alive != 1))),1);
draw_sprite_ext(sprite_index,image_index,_x,_y,image_xscale*(1-generalShake/12),image_yscale*(1-generalShake/12),image_angle,merge_color(c_white,c_red,min(1,generalShake/2+(global.alive != 1))),image_alpha);