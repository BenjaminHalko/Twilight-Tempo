	...
	if mouse_check_button_pressed(mb_left) {
		 if menuType == Menu.Main {
			 if normal.clicked() {
				...
				pb.updateText("HI-SCORE: "+string_replace_all(string_format(global.hiScore[0],4,0)," ","0"));
				event_user(0);
			} else if hard.clicked() {
				menuType = Menu.Hard;
				global.hardMode = true;
				audio_play_sound(snBlip,1,false);
				pb.updateText("HI-SCORE: "+string_replace_all(string_format(global.hiScore[1],4,0)," ","0"));
				event_user(0);
...
if(array_length(stars) < 150) array_push(stars,{
    ...
    y: random(HEIGHT),
	colNumber: irandom(2),
	alpha1: random_range(0.2,1),
	alpha2: random_range(0.2,1),
	alphaSpeed: random_range(1,5),
	alphaOffset: random(1),
	far: random(1),
	col: global.starColors[irandom(2)]
});